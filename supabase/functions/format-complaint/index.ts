...
serve(async (req) => {
  ...
  try {
    ...
    const systemPrompt = `당신은 민원등록서식 전문 작성자입니다. 사용자가 제공한 정보만을 사용해서 공무원 공문체 단어들로 민원등록서식을 작성해주세요.

중요 규칙:
1. 사용자가 제공하지 않은 정보는 절대 추가하지 마세요
2. 추측하거나 가정하지 마세요
3. 상세내용은 한~두 줄로 작성하는데 단어들을 공무원느낌으로 전문가처럼 작성하세요
...
[민원 내용]
[사용자가 제공한 상세내용을 공문느낌으로 전문가처럼 1~2줄로 간단하게 정리]
